...
RETURN testmain(PARAM* parametereter, int is_aided, std::vector<float> time_array, 
...
void publishMarkerCubes()
{
  ...
  for(int i = 0; i < MAPSIZE*2; i++)
  {
    for(int j = 0; j < MAPSIZE*2; j++)
    {
      ...
      if( return_struct.seen_map.seen[i][j] )
      {
...
void parseMap()
{
  ...
  string filename = "/home/jaeho-linux/hri2017/src/planner_compile/maps"; //This needs to be modified for each computer.....
...
{ 
  ...
  float coverage_ratio = msg->coverage_ratio;
  ...
  return_struct = testmain(&local_parameter, is_aided, time_array, sequence_array, is_fixed, coverage_ratio);  
  ...
  if(is_aided == 0 && MAP_NUM <= 5){
    ...
    RETURN return_tmp = testmain(&local_parameter, 1, time_array, seq_tmp, is_fixed, coverage_ratio);
