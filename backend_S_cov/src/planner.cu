...
Node Planner::makePlan()
{
  ...
  {
    for (int j = 0; j < node.seen_map.mapsize; j++)
    {
...
Node Planner::executePipeline()
{
  ...
  while (!nodes.empty()) {
    ...
    float* d_coverage_ratios;
    cudaMalloc(&d_coverage_ratios, sizeof(float)*nodesToExpand);
    dim3 stageA0Grid(nodesToExpand);
    plannerStageA0<<<stageA0Grid, 1>>>(pNodes, d_coverage_ratios);
...
/*
    for(int i = 0; i < nodesToExpand; i++)
    {
      if(h_coverage_ratios[i] > TARGET_RATIO)
      {
        ...
        if (currNode.cost + currNode.heuristic <= priorities[0]) { 
          currNode.coverage_ratio = h_coverage_ratios[i];
          return currNode; 
        }
      }
    }
